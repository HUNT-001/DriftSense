...
# Stage 2E — distortion-robust LER (diagnosis complete; one open blocker)

## Diagnosis: which distortion degrades the descriptor?

Measured directly as the fingerprint similarity between the reference and the
*true* location — no competitors, each distortion swept alone:

| axis | ideal | mild | moderate | extreme |
|---|---|---|---|---|
| **rotation** | 0.74 | 0.66 (1°) | 0.51 (2.5°) | **0.32 (5°)** |
| **scale** | 0.74 | 0.65 (1%) | 0.61 (3%) | **0.47 (7%)** |
| drift | 0.74 | 0.70 | 0.77 | 0.88 *(rises — harmless)* |
| noise | 0.78 (low) | — | 0.70 (med) | 0.60 (high) |

**Rotation is the steepest degrader, scale second.** Drift is harmless (it
even helps — the shear adds distinctiveness), and noise is moderate and not
geometrically fixable.

## Correcting my earlier "resampling always hurts" claim

That claim came from a test that bundled rotation+scale with *bilinear*
interpolation. Isolating it: de-rotating a candidate with **cubic**
interpolation *recovers* the descriptor —

| rotation | no correction | de-rotated (cubic) |
|---|---|---|
| 2.5° | 0.55 | **0.67** |
| 5.0° | 0.37 | **0.67** |

Rotation resampling is safe for LER because it displaces the smooth envelope
while cubic reconstruction preserves the fine lateral detail. The earlier
failure was really *scale* resampling with *linear* interpolation.

## But rotation correction alone doesn't move end-to-end — both are needed

Oracle (true-parameter) correction, full 1000×1000 search, hard tier:

| correction | Acc@5 | median err |
|---|---|---|
| none | 33% | 344 px |
| oracle rotation only | 33% | 161 px |
| **oracle rotation + scale** | **47%** | **58 px** |

Rotation alone pulls the winner onto a *nearer* replica (median 344→161) but
not onto the truth; only rotation **and** scale together clear the 5 px bar
(+14 points). They compound, and both must be corrected.

## The blocker: robust rotation estimation on periodic grids

Scale is estimable accurately — the period ratio gives median error 0.0095
against a ±0.07 range. Rotation is the problem:

| estimator | median | P90 | end-to-end (hard) |
|---|---|---|---|
| oracle | 0 | 0 | 47% |
| dominant FFT vector | **0.03°** | **5.8°** | **27%** |

The dominant-vector angle is excellent on median but has ~30% bad-axis
outliers (staggered-DRAM diagonal peaks; FinFET fin-vs-gate ambiguity), and a
single wrong angle corrupts that pair's entire de-warp — dragging estimated
rot+scale (27%) *below* even no correction (33%). A multi-peak angle-consensus
estimator was prototyped to beat the outliers but did not converge on real
FinFET/staggered spectra; it was removed rather than shipped worse than
baseline.

**The safe-fallback resolution.** Rather than wait for a perfect estimator, the
localizer scores each candidate as `max(unwarped, dewarped)` using the
consensus rotation + period-ratio scale. Because the unwarped score is always
available, a wrong `(θ, scale)` estimate can only *fail to help* — it can never
drag a candidate below baseline. This is leakage-safe (one global transform for
all candidates) and non-regressing by construction:

| config (hard) | Acc@5 | median err |
|---|---|---|
| lattice + LER, no de-warp | 25% | 378 px |
| **+ safe rot+scale de-warp** | **33%** | **222 px** |
| oracle rot+scale (ceiling) | 47% | 58 px |

So the safe de-warp recovers part of the oracle gain (+8 pts here, median
378→222) with zero regression risk, and the remaining gap to 47% is exactly the
robust-orientation blocker. `derotate` defaults OFF; enable per call.

Honest scope note: these end-to-end rows are 12–15 pairs; treat ±10% as noise.
The descriptor-degradation diagnosis and the scale-estimation accuracy are the
robust results.

## What's next (unchanged priority, now precisely scoped)

1. Robust orientation on periodic grids — the one blocker. Candidate methods:
   RANSAC over peak pairs constrained to a rectangular basis; a full 2-D
   lattice fit; or a Radon/structure-tensor cross-check to disambiguate the
   axis. Once median-good *and* outlier-free, the oracle 47% should be
   reachable.
2. Scale correction is ready (accurate, cheap) and pairs with it.
...
| 2E — Distortion-robust LER | diagnosed; safe de-warp 25%→33% (no regression); oracle ceiling 47%; robust grid orientation remains the gap |
...
python -m experiments.candidate_budget --tier hard      # Stage 2D budget curve
python -m experiments.descriptor_degradation            # Stage 2E diagnosis
python -m experiments.rotation_study                    # Stage 2C orientation
```
